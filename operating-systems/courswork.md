...
1. What are the advantages of using dynamic loading? (6 marks)
    > past types of loading used static loading which 
    > lower startup time
    > libraries are linked rather than copied (lower memory use)
    > when libraries are updated the updates are available to the system without reloading
1. Explain the basic method for implementing paging. (8 marks)
    > Logical memory addresses are mapped into a Virtual memory space instead of directly into physical memory.
    > This level of indirection allows for a logical address to be mapped somewhere else in memory
    > For this to work both logical and physical memory needs to be mapped into the same address space.
    > In the kernel there exists a table that maps a logical address for a given process to the related physical address.
    > Whenever a TLB doesn't hold a reference to the required physical block a trap is issued and the kernel is passed the information about the memory access request.
    > The Kernel looks up the page table for that process which usually contains a virtual memory size, and an indexable array of physical memory blocks.
    > If the requested block is above the virtual block accessible by the process, the process is terminated.
    >  and determines if it's allowed to access that memory and if so what the address is.
    > If the request is allowed the kernel, passes that page table information back to the TLB and context switches into the processes at the instruction where the trap was called.
    > This time when the instruction is called the TLB is available and the memory call is.
1. Briefly describe the segmentation memory management scheme. How does it differ from the paging memory management scheme in terms of the user’s view of memory? (8 marks)
    > Before segmentation existed an older method called continual
    > Segmentation uses the 
    > yes. With the paging scheme the users view of memory is that of one large array.
1. Explain the distinction between a demand-paging system and a paging system with swapping. (8 marks)
    > demand-paging is when a process requests a page and it doesn't exist in memory
    > page swapping is when a page isn't used for a while and the OS moves that page to disk in favour of another page
    > both systems are complimentary and can be used together.
1. How does the second-chance algorithm for page replacement differ from the FIFO page replacement algorithm? (8 marks)
    ...
    > This results in recently used pages being reclaimed.
    > The second-chance has a single access bit per page.
    > Every time the page is accessed the bit is set.
    > A frame is needed in order to bring a page into memory from the backing store.
    ...
    > This differs from the FIFO page replacement algorithm in that there is an additional factor in deciding if the 
...
1. Explain how copy-on-write operates. (8 marks)
    > Copy-on-write (COW) is a way to mark a specific page in memory.
    > This often occurs when a process forks another process, or thread.
    > Instead of making a copy of all the pages associated with the process, only the page table of the process is copied.
    > By only copying the page-table and not the entire memory space, forking a process becomes much faster and consumes less memory.
    > Many of these pages are read only and can be shared easily between processes.
    > However, some are process state and can be written.
    > Without any other type of mechanism a write by one process would change the memory state of another process, so usually this method could not be used for RW areas of memory.
    > When a page is marked as COW the first time a process accesses this memory a copy is made.
    > The new copy is no longer marked as COW and the process initiating the write can now make changes to the page without affecting other processes.
    > COW has the benefit of reducing memory usage, while maintaining process boundaries.
...
1. How is a hash table superior to a simple linear list structure? What issue must be handled by hash table implementation? (8 marks)
    > Size of hash table and hashing algorithm decide
...
1. Explain the concepts of a bus and a daisy chain. Indicate how these concepts are related. (8 marks)
    ...
    > a single controller usually sends controls over part of the bus that single which device is being targeted and what action that device should perform.
    > The data being passed to or from the controller is generally on a separate set of wires.
    > a daisy chain is the concept of stringing together IO devices.
    > Each device has an inbound and outbound port
    > a daisy chain generally implements a bus
1. What are the three reasons that buffering is performed? (6 marks)
    > group interrupts in order to reduce time spent context switching
    > provide a safe area for data to be kept while waiting for an interrupt
    > 
